...
def makenuSQuIDS(ene,zen,flav,n_type,mutau=0.0,tautau=0.0):
  if(n_type>0):
...
class nsiFit(object):
  ...
  def __init__(self):
    ...
    self.eCounts =  dict.fromkeys(self.nsi_keys) 
    self.tCounts =  dict.fromkeys(self.flav_keys) 
    ...
    self.spl_keys = [(p,q) for p in self.flav_keys for q in ['domeff','holeice']]
    self.syst_splines = {key: [[None for i in self.z_bins[:-1]] for j in self.e_bins[:-1]] for key in self.spl_keys} 
  ...
  def CalcNSIWeights(self,e_mt,e_tt,flav):
    ...
    if(self.use_splines):
      ...
      for i in xrange(len(self.t_nEnergy[flav])):
        ...
        eeW,emW,etW = makenuSQuIDS(self.t_nEnergy[flav][i],self.t_nZen[flav][i],'e',self.t_nType[flav][i],e_mt,e_tt)
        meW,mmW,mtW = makenuSQuIDS(self.t_nEnergy[flav][i],self.t_nZen[flav][i],'mu',self.t_nType[flav][i],e_mt,e_tt)
        if(flav[0]=='e'):
          e_nsi_weights[i] = eeW 
          m_nsi_weights[i] = meW 
        if(flav[0]=='mu'):
...
#Create binned total systematic differences
  def makeSysDiffCounts(self,domeff,holeice,flav):
    ...
    for ene in xrange(len(self.e_bins)-1):
      for zen in xrange(len(self.z_bins)-1):
        ...
        sysCounts[ene][zen] = (float(self.syst_splines[(flav,'domeff')][ene][zen](domeff))*float(self.syst_splines[(flav,'holeice')][ene][zen](holeice)))
...
#Calculate LLH for different values of epsilon_mutau and epsilon_tautau, DOM efficiency (domeff), and Hole Ice variations (holeice)
  def llhFunc(self,norm=1.0,e_mt=0.0,e_tt=0.0,domeff=0.91,holeice=50,gamma=0,norm_e=1):
    rCounts = None
    sCounts = None
    dCounts = None
    rel_qe = 3.0
...
#Base counts
    for flav in [(p,'domeffm9') for p in self.flav_keys]:
      if(rCounts!=None):
        sCounts = self.makeSysDiffCounts(domeff,holeice,flav[0])
        rCounts += numpy.nan_to_num(sCounts)*norm*self.makeRefCounts(e_mt,e_tt,flav,gamma,norm_e) 
        dCounts +=self.tCounts[flav[0]]
      else:
        sCounts = self.makeSysDiffCounts(domeff,holeice,flav[0])
        rCounts = numpy.nan_to_num(sCounts)*norm*self.makeRefCounts(e_mt,e_tt,flav,gamma,norm_e)
        dCounts = self.tCounts[flav[0]]
...
#Poisson Likelihood 
    LLH = sum(rCounts[rCounts>=0] - (dCounts[rCounts>=0])*log(rCounts[rCounts>=0]))

    LLH += (0.5*((norm_e-1.)/sigma_norm_e)**2 +
            0.5*(gamma/sigma_gamma)**2 +
            0.5*(rel_qe/sigma_rqe)**2 +
            0.5*((domeff-0.91)/sigma_domeff)**2 +
            0.5*(((1/holeice)-0.02)/sigma_holeice)**2 
           )

# Chi-Squared Likelihood
    #LLH2 = sum(((rCounts[rCounts>=0] - dCounts[self.rCounts>=0])*(rCounts[rCounts>=0] - dCounts[self.rCounts>=0]))/(rCounts[rCounts>=0]))
    #print "LLH:",LLH
    #print "CHi2:",LLH2
    return LLH
...
#Make splines for dom efficiency. 'base' has DOM efficiency 1.0, values are 100*(difference from 1.0)
    ...
    sel_nsi_keys = [i for i in self.nsi_keys if i[1] in sel_keys] 
    diffCounts = dict.fromkeys(sel_nsi_keys) 
    wCounts = dict.fromkeys(sel_nsi_keys) 
    for syst in sel_nsi_keys: 
      diffCounts[syst] = self.eCounts[syst]/self.eCounts[(syst[0],'domeffm9')]
#      print "Counts",self.eCounts[syst],self.eCounts[(syst[0],'domeffm9')]
      wCounts[syst] = diffCounts[syst]*(numpy.sqrt(((numpy.sqrt(self.eCounts[syst])/self.eCounts[syst])*(numpy.sqrt(self.eCounts[syst])/self.eCounts[syst])) + ((numpy.sqrt(self.eCounts[(syst[0],'domeffm9')])/self.eCounts[(syst[0],'domeffm9')])*(numpy.sqrt(self.eCounts[(syst[0],'domeffm9')])/self.eCounts[(syst[0],'domeffm9')]))))
    for ene in xrange(len(self.e_bins)-1):
      for zen in xrange(len(self.z_bins)-1):
        for flav in self.flav_keys:
          syst_diffs = []    
          w_vals = []    
          for syst in sel_keys: 
            syst_diffs.append(diffCounts[(flav,syst)][ene][zen])     
            w_vals.append(wCounts[(flav,syst)][ene][zen])
#            print 'DE Diffs',flav,syst,self.e_bins[ene],self.z_bins[zen],diffCounts[(flav,syst)][ene][zen],wCounts[(flav,syst)][ene][zen]
        #k_spline = scipy.interpolate.KroghInterpolator([0.82,0.86,0.91,0.95,1.0,1.05,1.1],syst_diffs)   
          ...
          self.syst_splines[(flav,'domeff')][ene][zen] = k_spline

#          fig = plt.figure()
#          plt.plot([0.8,0.82,0.84,0.86,0.88,0.90,0.92,0.94,0.96,0.98,1.0,1.02,1.04,1.06,1.08,1.1],k_spline([0.8,0.82,0.84,0.86,0.88,0.90,0.92,0.94,0.96,0.98,1.0,1.02,1.04,1.06,1.08,1.1])) 
#        k_spline = orthopoly1d(np.polyfit([0.82,0.86,0.91,0.95,1.0,1.05,1.1],syst_diffs,2)) 

#          for i in numpy.linspace(0.8,1.1,30):
#          plt.errorbar([0.82,0.86,0.91,0.95,1.0,1.05,1.1],syst_diffs,yerr=w_vals)
#          p_name = 'DOMEff-%d-%d-%s.png' % (self.e_bins[ene],self.z_bins[zen],flav)
#          pylab.savefig(p_name)
#          plt.close(fig)
#            print 'DomEff',i,flav,self.e_bins[ene],self.z_bins[zen],k_spline(i)
    ...
    for ene in xrange(len(self.e_bins)-1):
      for zen in xrange(len(self.z_bins)-1):
        for flav in self.flav_keys:
          ...
          self.syst_splines[(flav,'holeice')][ene][zen] = k_spline


#Fit the simulation to (mock)D/data 
  def fitFunc(self,e_mt=0.0,e_tt=0.0,use_splines=True):
...
#Make sets and NSI weights for all flavors and systematics
      if(self.eCounts[flav]!=None):
        print flav[1]
        self.eCounts[flav] += self.makeRefCounts(e_mt,e_tt,flav)
      else:
        print flav[1]
        self.eCounts[flav] = self.makeRefCounts(e_mt,e_tt,flav)
...
#Set the "true" Counts
      if(flav[1]=='domeffm9'):
        if(self.tCounts[flav[0]]!=None):
          self.tCounts[flav[0]] += 1.0*self.makeRefCounts(0.0,0.0,flav,0,1,True) 
        else:
          self.tCounts[flav[0]] = 1.0*self.makeRefCounts(0.0,0.0,flav,0,1,True)
...
#Systematics for test counts
    for flav in self.flav_keys:
      self.tCounts[flav] = numpy.nan_to_num(self.makeSysDiffCounts(1.0,50,flav))*self.tCounts[flav] 
...
#Make some fits
my_fit = nsiFit()
my_fit.fitFunc(float(sys.argv[1]),float(sys.argv[2]),True)
