...
file_list1 = glob.glob(sys.argv[2])
llh = []
for lfile in file_list1:
  try:
    llh_file = pickle.load(open(lfile, "rb"))
    mt = float("%.5f" % llh_file['result']['mt'])
    tt = float("%.5f" % llh_file['result']['tt'])
    #print mt,tt
    llh.append(llh_file['result']['llh'])
...
plt.plot([sys.argv[3]], [sys.argv[4]], 'k.', markersize=14.0)
